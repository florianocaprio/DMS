...
- **Primo avvio (setup)**: al boot, se nel database non esiste alcun amministratore, viene seedato un **admin di default** (`username: admin`, senza password) tramite `ensureDefaultAdmin()`. Finché quell'admin locale non ha una password, l'app entra in modalità setup e mostra — senza login — la schermata di impostazione password tramite gli endpoint pubblici `GET/POST /api/auth/bootstrap` (GET → `{setupMode, username}`; POST imposta la password ≥8 caratteri, fa **auto-login** con cookie di sessione e blocca l'endpoint, atomico via `pg_advisory_xact_lock`). Gli admin Clerk/Google (username `null`) **non** attivano mai il setup. Impostata la password, l'accesso richiede le credenziali e tutto il resto si configura nell'app.
